...
// Interface for each design package variation
interface Variation {
...
// Interface for the complete project object
interface Project {
  slug: string;
  title: string;
  description: string;
...
export default async function ProjectDetail({
  ...
  try {
    // Fetch all design projects from API
    const response = await axios.get(`${process.env.NEXT_PUBLIC_SITE_URL}/api/design-project`);
    const allProjects: Project[] = response.data;

    // Find the project that matches the URL slug
    const project = allProjects.find((p) => p.slug === params.slug);
    if (!project) return notFound(); // If not found, render 404 page

    // Render the project detail UI
    return (
      <>
    ...
    return notFound(); // In case of API error, fallback to 404
  }
}
