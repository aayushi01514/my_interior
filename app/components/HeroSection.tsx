...
const HeroSection: React.FC<HeroSectionProps> = ({ title, description, linkname, linksrc, imgsrc, imgalt }) => {
    return (
        <main>
            <div className="flex flex-col md:flex-row gap-20 md:gap-10 mt-0 p-5">
                <div className=" flex-col items-start justify-start p-5 md:p-10 rounded-2xl inline-block">
                    ...
                    {
                        linkname && linksrc && (
                            <Link href={linksrc} className="text-blue-600 hover:underline text-xl">
                                {linkname}
